...
const POM_strSpecialThree = 'АБЦДЕФГХИЙКЛМНОПQРСТУВWХЫЗ';
const POM_strSpecialFour = 'абцдефгхийклмнопqрстувwхыз';
const POM_strSpecialFive = 'あおゔぎばちまかたげでぷそ';
...
const POM_strSpecialThree_Default = false;
const POM_strSpecialFour_Default = false;
const POM_strSpecialFive_Default = false;
...
function POM_generatePassword() {
    ...
    if (mkLengthField) {
        if (mkLengthField.value >= POM_minLength && mkLengthField.value <= POM_maxLength) {
            ...
            if (document.getElementById('mk-special-three-select').checked) {
                passwordSource += (POM_strSpecialThree + POM_strSpecialThree);
            }
            if (document.getElementById('mk-special-four-select').checked) {
                passwordSource += (POM_strSpecialFour + POM_strSpecialFour);
            }
            if (document.getElementById('mk-special-five-select').checked) {
                passwordSource += (POM_strSpecialFive + POM_strSpecialFive);
            }
            if (passwordSource.length>0) {
                ...
                /* See if password contains specialthree characters */
                searchStr = '[' + POM_escapeRegExp(POM_strSpecialThree) + ']';
                if (POM_debug) {
                    console.log('Using ' + searchStr + ' to scan password');
                }
                re = new RegExp(searchStr);
                if (re.test(passwordGen)) {
                    scoreP++;
                    if (POM_debug) {
                        console.log('Password contains specialThree');
                    }
                } else {
                    if (POM_debug) {
                        console.log('Password does NOT contain specialThree');
                    }
                }
                /* See if password contains specialfour characters */
                searchStr = '[' + POM_escapeRegExp(POM_strSpecialFour) + ']';
                if (POM_debug) {
                    console.log('Using ' + searchStr + ' to scan password');
                }
                re = new RegExp(searchStr);
                if (re.test(passwordGen)) {
                    scoreP++;
                    if (POM_debug) {
                        console.log('Password contains specialFour');
                    }
                } else {
                    if (POM_debug) {
                        console.log('Password does NOT contain specialFour');
                    }
                }
                /* See if password contains specialfive characters */
                searchStr = '[' + POM_escapeRegExp(POM_strSpecialFive) + ']';
                if (POM_debug) {
                    console.log('Using ' + searchStr + ' to scan password');
                }
                re = new RegExp(searchStr);
                if (re.test(passwordGen)) {
                    scoreP++;
                    if (POM_debug) {
                        console.log('Password contains specialFive');
                    }
                } else {
                    if (POM_debug) {
                        console.log('Password does NOT contain specialFive');
                    }
                }
...
function POM_initialSetup() {
    ...
    document.getElementById('mk-special-three-select').checked = POM_strSpecialThree_Default;
    document.getElementById('mk-special-four-select').checked = POM_strSpecialFour_Default;
    document.getElementById('mk-special-five-select').checked = POM_strSpecialFive_Default;
    ...
    document.getElementById("gen-special-three").innerText = POM_strSpecialThree;
    document.getElementById("gen-special-four").innerText = POM_strSpecialFour;
    document.getElementById("gen-special-five").innerText = POM_strSpecialFive;
    ...
    if (POM_changeIsClick) {
        ...
        document.getElementById('mk-special-three-select').addEventListener('click', POM_generatePassword);
        document.getElementById('mk-special-four-select').addEventListener('click', POM_generatePassword);
        document.getElementById('mk-special-five-select').addEventListener('click', POM_generatePassword);
    }
